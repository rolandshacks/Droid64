...
public class Util {
    ...
    */
    ...
    public static Rect matchSize(int width, int height) {
        int zoom_factor = Math.min(width / (EmuControl.DISPLAY_X-44),
                                   height / (EmuControl.DISPLAY_Y-80));
        if (zoom_factor < 1)
        {
            zoom_factor = 1;
        }
        ...
        int outW = EmuControl.DISPLAY_X  * zoom_factor;
        int outH = EmuControl.DISPLAY_Y  * zoom_factor;
