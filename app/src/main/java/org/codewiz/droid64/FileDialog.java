...
import org.codewiz.droid64.view.ObjectListView;
...
public class FileDialog extends DialogFragment {
    ...
    private DiskFilter currentDiskFilter;
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (null != setList) {
            ...
            DiskFilter noDiskFilter = new DiskFilter('\0', '\0', "All");
            setList.add(noDiskFilter);

            setList.add(new DiskFilter("snap", "Snapshots"));

            setList.add(new DiskFilter('0', '9'));
            for (char c = 'a'; c <= 'z'; c++) {
                setList.add(new DiskFilter(c));
            }

            currentDiskFilter = (DiskFilter) setList.getItemAtPosition(lastSelectedFilterPos);
    ...
    private void handleSetClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (null != item && item instanceof DiskFilter) {
            currentDiskFilter = (DiskFilter) item;
    ...
    private void handleItemClick(AdapterView<?> parent, View view, int position, long id, boolean longClick) {
        ...
        Object item = list.getItemAtPosition(position);
        if (null != item && item instanceof DiskImage) {
            DiskImage diskImage = (DiskImage) item;
            logger.info(diskImage.getUrl());

            if (longClick && diskImage.getType() == DiskImage.TYPE_SNAPSHOT) {
    ...
    private void editSnapshot(DiskImage snapshotImage) {
    ...
    public interface OnDiskSelectHandler {
        void onDiskSelect(DiskImage diskImage, DiskFilter diskFilter, boolean longClick);
    }
}
