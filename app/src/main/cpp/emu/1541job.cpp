...
void Job1541::open_d64_file(char *filepath)
{
	...
	const uint8* d64Buffer = getImageData();
...
*/
	...
	if (d64Buffer != NULL) {
		...
		// Check length
		if (getImageSize() < NUM_SECTORS * 256) {
			return;
		}

		// x64 image?
		if (d64Buffer[0] == 0x43 && d64Buffer[1] == 0x15 && d64Buffer[2] == 0x41 && d64Buffer[3] == 0x64)
			image_header = 64;
		else
			image_header = 0;
		...
		if (!image_header && size == NUM_SECTORS * 257) {
			memcpy(error_info, d64Buffer + NUM_SECTORS * 256, NUM_SECTORS);
		};
...
bool Job1541::read_sector(int track, int sector, uint8 *buffer)
{
	...
	const uint8* d64Buffer = getImageData();
	if (NULL == d64Buffer) return false;

	// Convert track/sector to byte offset in file
	if ((offset = offset_from_ts(track, sector)) < 0)
		return false;

	if (offset + image_header > getImageSize())
		return false;

	memcpy(buffer, d64Buffer + offset + image_header, 256);
...
bool Job1541::write_sector(int track, int sector, uint8 *buffer)
{
	...
	memcpy(const_cast<uint8*>(d64Buffer + offset + image_header), buffer, 256);
...
void Job1541::attachImage()
{
	detachImage();

	const uint8* imageData = TheC64->getDiskImage();
	int imageSize = TheC64->getDiskImageSize();
	...
	d64Buffer = new uint8[imageSize];

	memcpy(d64Buffer, imageData, imageSize);

	d64Size = imageSize;
}

void Job1541::detachImage()
{
	if (NULL != d64Buffer)
	{
		delete [] d64Buffer;
...
const uint8* Job1541::getImageData()
{
	return d64Buffer;
}

int Job1541::getImageSize()
{
	return d64Size;
}
