...
static int disass_line(uint16 adr, uint8 op, uint8 lo, uint8 hi)
{
	...
	switch (mode) {
		...
		case A_REL:
			fprintf(fout, "%04x", ((adr + 2) + (int8)lo) & 0xffff);
			break;
...
static void view_cia_state(void)
{
	...
	fprintf(fout, "TOD         : %x%x:%x%x:%x%x.%x %s\n",
	...
	fprintf(fout, "Alarm       : %x%x:%x%x:%x%x.%x %s\n",
...
static void view_vic_state(void)
{
	...
	fprintf(fout, "X scroll          : %d\n", vs.ctrl2 & 7);
	fprintf(fout, "Y scroll          : %d\n", vs.ctrl1 & 7);
	fprintf(fout, "Horizontal border : %d columns\n", vs.ctrl2 & 8 ? 40 : 38);
	fprintf(fout, "Vertical border   : %d rows\n\n", vs.ctrl1 & 8 ? 25 : 24);
