...
void D64Drive::open_close_d64_file(char *d64name)
{
	...
	if (d64name[0]) {
        ...
        int d64Size = getImageSize();

		if (d64Buffer != NULL && d64Size >= NUM_SECTORS * 256) {
			// x64 image?
			...
			if (!image_header && d64Size == NUM_SECTORS * 257) {
...
bool D64Drive::read_sector(int track, int sector, uint8 *buffer)
{
    ...
    int pos = offset + image_header;
    if (pos + 256 >= d64Size) {
		set_error(ERR_ILLEGALTS);
        return false;
    }

    memcpy(buffer, d64Buffer + pos, 256);
