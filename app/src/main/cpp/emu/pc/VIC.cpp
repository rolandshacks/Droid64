...
void MOS6569::init_display_colors(uint8 *colors)
{
	for (int i=0; i<256; i++)
    {
		colors[i] = i & 0x0f;
...
MOS6569::MOS6569(C64 *c64, MOS6510 *CPU, uint8 *RAM, uint8 *Char, uint8 *Color)
	: ram(RAM), char_rom(Char), color_ram(Color), the_c64(c64), the_cpu(CPU)
{
	int i;
    ...
    // Alloc buffer
    screen_buffers = new uint8[buffer_size*2];
    memset(screen_buffers, 0, buffer_size*2);

    back_buffer  = screen_buffers;
    front_buffer = screen_buffers + buffer_size;

	chunky_line_start = getBackBuffer();
...
MOS6569::~MOS6569()
{
    if (NULL != screen_buffers)
    {
        delete [] screen_buffers;
        screen_buffers = NULL;
    }
...
uint8* MOS6569::getFrontBuffer()
{
    return front_buffer;
}

uint8* MOS6569::getBackBuffer()
{
    return back_buffer;
}

int MOS6569::getBufferWidth()
{
    return DISPLAY_X;
}

int MOS6569::getBufferSize()
{
    return getBufferWidth() * DISPLAY_Y;
}

void MOS6569::swapBuffers()
{
    uint8* s = back_buffer;
